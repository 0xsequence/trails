            ...
            return {
                ...
                queryFn: async () => {
                    ...
                    if (opStatus.status === "confirmed") {
                        ...
                        console.warn(`[trails-sdk] ⚠️ Unexpected relayer status "${originalStatus}" for ${opHashToPoll}:`, opStatus);
                        newStatusEntry = {
                            status: "unknown",
                        };
                    }
                ...
                retry: (failureCount, error) => {
                    if (failureCount >= 30) {
                        console.error(`[trails-sdk] ❌ Giving up on transaction ${opHashToPoll} after 3 failed API attempts:`, error);
                        return false;
                    }
                    return true;
                },
