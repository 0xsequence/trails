import { createPublicClient, encodeFunctionData, encodePacked, http, pad, parseAbi, parseEther, parseGwei, toHex, zeroAddress, } from "viem";
import { createBundlerClient, createPaymasterClient, prepareUserOperation, } from "viem/account-abstraction";
import { generatePrivateKey, privateKeyToAccount } from "viem/accounts";
import { getPermitCalldata, getPermitSignature, getTransferCalldata, getTransferFromCalldata, } from "./gasless.js";
...
export function packPayload(payload) {
    ...
    const encodedCalls = payload.calls.map((call) => {
        ...
        return encodePacked(["uint8", "address", "uint256", "uint256", "bytes"], [callFlags, call.to, call.value, call.gasLimit, call.data]);
    });
    return encodePacked(["uint8", "bytes20", "uint8", ...Array(numCalls).fill("bytes")], [
        globalFlag,
...
// --- Main logic ---
export async function getDelegatorSmartAccount({ publicClient, }) {
    ...
    console.log("[trails-sdk] Smart account address:", delegatorSmartAccount.address);
    console.log("[trails-sdk] delegatorSmartAccount.address", delegatorSmartAccount.address);
    return delegatorSmartAccount;
}
export async function getPaymasterGaslessTransaction({ walletClient, chain, tokenAddress, amount, recipient, delegatorSmartAccount, }) {
    // Initialize clients
    ...
    const connectedAccount = walletClient.account.address;
...
export async function sendPaymasterGaslessTransaction({ walletClient, publicClient, chain, paymasterUrl, delegatorSmartAccount, calls, }) {
    try {
        if (!walletClient.account) {
            throw new Error("No account found");
        }
        ...
        if (paymasterUrl) {
            ...
            const signedUserOp1 = await delegatorSmartAccount.signUserOperation(userOp1);
            userOp1.signature = signedUserOp1;
...
export async function stakeOnEntryPoint({ relayerClient, relayerAccount, delegatorAddress, chain, }) {
