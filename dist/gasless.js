...
export function getPermitCalldata(connectedAccount, spender, amount, deadline, signature) {
...
export function getTransferFromCalldata(connectedAccount, spender, amount) {
...
export async function getPermitSignature(publicClient, walletClient, connectedAccount, delegatorSmartAccountAddress, tokenAddress, amount, chain, deadline = BigInt(Math.floor(Date.now() / 1000) + 3600)) {
...
export function getDepositToIntentWithPermitCalldata(params) {
    ...
    const { user, token, amount, permitAmount, intentAddress, deadline, v, r, s, // permit signature
    sigV, sigR, sigS, // intent signature
     } = params;
...
export function getDepositToIntentWithoutPermitCalldata(params) {
    ...
    const { user, token, amount, intentAddress, deadline, sigV, sigR, sigS } = params;
    return encodeFunctionData({
        abi,
...
export async function getDepositToIntentCalls({ publicClient, walletClient, account, intentEntrypoint, depositTokenAddress, depositTokenAmount, depositRecipient, chain, }) {
    ...
    if (needsApproval) {
        approvalSignature = await getPermitSignature(publicClient, walletClient, account.address, intentEntrypoint, depositTokenAddress, permitAmount, chain, deadline);
    }
    const { v: signedIntentV, r: signedIntentR, s: signedIntentS, } = await signIntent(walletClient, {
...
export async function signIntent(client, { user, token, amount, intentAddress, deadline, chainId, contractAddress, }) {
    const domain = {
