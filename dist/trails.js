...
import { calculateIntentAddress, getIntentCallsPayloads as getIntentCallsPayloadsFromIntents, } from "./intents.js";
...
export function useTrails(config) {
    const { account, disableAutoExecute = false, env, useV3Relayers = true, sequenceProjectAccessKey, } = config;
    ...
    const [trailsInfos, setTrailsInfos] = useState(null);
    ...
    const calculatedIntentAddress = useMemo(() => {
        ...
        return calculateIntentAddress(account.address, intentCallsPayloads, trailsInfos, trailsFee?.quoteProvider); // TODO: Add proper type
    ...
    const commitIntentConfigMutation = useMutation({
        mutationFn: async (args) => {
            ...
            try {
                ...
                const calculatedAddress = calculateIntentAddress(args.mainSignerAddress, args.calls, // TODO: Add proper type
                args.trailsInfos, // TODO: Add proper type
                args.quoteProvider);
    ...
    const sendMetaTxnMutation = useMutation({
        mutationFn: async ({ selectedId }) => {
            ...
            const intentAddress = calculateIntentAddress(account.address, intentCallsPayloads, trailsInfos, trailsFee.quoteProvider); // TODO: Add proper type
    ...
    useEffect(() => {
        ...
        metaTxns.forEach(async (metaTxn) => {
            ...
            if (processedTxns.current.has(operationKey)) {
                console.log(`[Trails] MetaTxn ${operationKey}: Already processed, skipping`);
                return;
            }
            const monitorStatus = metaTxnMonitorStatuses[operationKey];
            if (!monitorStatus || monitorStatus.status !== "confirmed") {
                console.log(`[Trails] MetaTxn ${operationKey}: Status not confirmed, skipping`);
                return;
            }
            // Type assertion since we know it exists when status is "confirmed"
            const transactionHash = monitorStatus.transactionHash;
            if (!transactionHash) {
                console.log(`[Trails] MetaTxn ${operationKey}: No transaction hash, skipping`);
                return;
            }
            console.log(`[Trails] MetaTxn ${operationKey}: Processing transaction ${transactionHash}`);
            ...
            try {
                ...
                if (receipt && typeof receipt.blockNumber === "bigint") {
                    ...
                    console.log(`[Trails] MetaTxn ${operationKey}: Got block timestamp ${block.timestamp}`);
                    ...
                    console.warn(`[Trails] MetaTxn ${operationKey}: No block number in receipt`);
                    setMetaTxnBlockTimestamps((prev) => ({
                        ...prev,
                        [operationKey]: {
                            timestamp: null,
