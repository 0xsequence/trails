...
import { TRAILS_CCTP_SAPIENT_SIGNER_ADDRESS, TRAILS_LIFI_SAPIENT_SIGNER_ADDRESS, TRAILS_RELAY_SAPIENT_SIGNER_ADDRESS, } from "./constants.js";
...
export function useTrails(config) {
    const { account, disableAutoExecute = false, env, useV3Relayers = getSequenceUseV3Relayers(), sequenceProjectAccessKey, } = config;
    ...
    const lastQueuedCctpSourceTxHash = useRef(null);
    ...
    useEffect(() => {
        ...
        metaTxns.forEach(async (metaTxn, index) => {
            const operationKey = `${metaTxn.chainId}-${metaTxn.id}`;
                ...
                if (receipt && typeof receipt.blockNumber === "bigint") {
                    ...
                    try {
                        ...
                        const providerFromArgs = createIntentMutation.variables?.provider
                            ...
                            : undefined;
                        const isCctp = providerFromQuote === "cctp" || providerFromArgs === "cctp";
                        const originChainIdForCctp = createIntentMutation.variables?.originChainId;
                        const destinationChainIdForCctp = createIntentMutation.variables?.destinationChainId;
                        ...
                        console.log("[Trails] createIntentMutation.variables", createIntentMutation.variables);
                        console.log("[Trails] createIntentMutation.variables.provider", createIntentMutation.variables?.provider);
                        console.log("[Trails] createIntentMutation.variables.originChainId", createIntentMutation.variables?.originChainId);
                        console.log("[Trails] createIntentMutation.variables.destinationChainId", createIntentMutation.variables?.destinationChainId);
                        console.log("[Trails] lastQueuedCctpSourceTxHash", lastQueuedCctpSourceTxHash.current);
                        console.log("[Trails] transactionHash", transactionHash);
                        console.log("[Trails] originChainIdForCctp", originChainIdForCctp);
                        console.log("[Trails] destinationChainIdForCctp", destinationChainIdForCctp);
                        if (isCctp &&
                            apiClient &&
                            transactionHash &&
                        ...
                        console.error(`[trails-sdk] Queue CCTP on meta-tx error (${operationKey}):`, err);
                    }
                }
                else {
