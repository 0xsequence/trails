...
export const TRAILS_CONTRACT_PLACEHOLDER_AMOUNT = 0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefn;
const proxyCallers = {
    [chains.base.id]: "0x4F54Dc2C6bCa2D01e066fa36f4CFdA96B544DD20",
    [chains.arbitrum.id]: "0x8c070e800D0e0c33A0A4ceef7A91677410ed9444",
};
export function getProxyCallerAddress(chainId) {
...
export function encodeProxyCallerCalldata({ token, target, calldata, amountOffset, }) {
    // Convert BigInt to bytes32 format
    const placeholderBytes32 = `0x${TRAILS_CONTRACT_PLACEHOLDER_AMOUNT.toString(16).padStart(64, "0")}`;
...
export function wrapCalldataWithProxyCallerIfNeeded({ token, target, calldata, originChainId, destinationChainId, amount, originTokenAddress, destinationTokenAddress, }) {
    const isSameToken = originTokenAddress?.toLowerCase() === destinationTokenAddress?.toLowerCase();
    if (originChainId === destinationChainId && isSameToken) {
        const amountHex = BigInt(amount).toString(16).padStart(64, "0"); // 32-byte hex (no 0x)
        const placeholderHex = TRAILS_CONTRACT_PLACEHOLDER_AMOUNT.toString(16).padStart(64, "0");
        if (!calldata.includes(placeholderHex)) {
            return null;
        }
        const calldataWithAmount = calldata.replace(placeholderHex, amountHex);
        return {
            encodedCalldata: calldataWithAmount,
    ...
    const amountOffset = getAmountOffset(calldata, BigInt(TRAILS_CONTRACT_PLACEHOLDER_AMOUNT));
    if (amountOffset === -1) {
        return null;
    }
