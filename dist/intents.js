...
import { TRAILS_LIFI_ATTESATION_SIGNER_ADDRESS, TRAILS_LIFI_SAPIENT_SIGNER_LITE_ADDRESS, TRAILS_RELAY_SAPIENT_SIGNER_LITE_ADDRESS, } from "./constants.js";
...
export function calculateIntentAddress(mainSigner, calls, executionInfosArg, sapientType = "relay") {
    ...
    const calculatedAddress = calculateIntentConfigurationAddress(Address.from(mainSigner), coreCalls, context, TRAILS_LIFI_ATTESATION_SIGNER_ADDRESS, coreExecutionInfos, sapientType);
...
export function hashIntentParams({ userAddress, nonce, originTokens, destinationCalls, destinationTokens, }) {
    ...
    for (let i = 0; i < destinationCalls.length; i++) {
        ...
        if (!callPayload)
            throw new Error(`DestinationCalls[${i}] is nil`);
        const currentDestCallPayloadHashBytes = Payload.hash(TRAILS_LIFI_ATTESATION_SIGNER_ADDRESS, BigInt(callPayload.chainId), {
            type: "call",
