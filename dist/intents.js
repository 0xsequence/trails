...
export function calculateIntentAddress(mainSigner, calls, executionInfosArg, sapientType = "relay") {
    ...
    console.log("[trails-sdk] Transformed coreExecutionInfos:", JSON.stringify(coreExecutionInfos, (_, v) => (typeof v === "bigint" ? v.toString() : v), 2));
    const calculatedAddress = calculateIntentConfigurationAddress(Address.from(mainSigner), coreCalls, context, ATTESATION_SIGNER_ADDRESS, coreExecutionInfos, sapientType);
...
export function getTrailsExecutionInfoHash(executionInfos, attestationAddress) {
    ...
    if (executionInfos && executionInfos.length > 0) {
        if (attestationSigner) {
            const sapientSignerLeaf = {
                ...
                weight: 1n,
                imageHash: getTrailsExecutionInfoHash(executionInfos, attestationSigner),
            };
