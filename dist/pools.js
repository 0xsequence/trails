...
const WRAPPED_TOKEN_GATEWAY_ADDRESS = {
...
export function getAaveV3WrappedTokenGatewayAddress(chainId) {
...
export function usePools() {
    ...
    const pools = useMemo(() => {
        ...
        return markets.flatMap((market) => {
            ...
            console.log("[trails-sdk] Supply reserves type:", typeof market.supplyReserves);
            console.log("[trails-sdk] Supply reserves length:", market.supplyReserves?.length);
            if (!market.supplyReserves || market.supplyReserves.length === 0) {
                console.log("[trails-sdk] No supply reserves found for market:", market.chain.chainId);
                return [];
            }
            return (market.supplyReserves
                ?.filter((reserve) => {
                // Check if reserve has supply info and is not frozen/paused
                const hasSupplyInfo = reserve.supplyInfo && !reserve.isFrozen && !reserve.isPaused;
                const hasLiquidity = parseFloat(reserve.size?.usd || "0") > 0;
                console.log(`[trails-sdk] Reserve ${reserve.underlyingToken?.symbol}: hasSupplyInfo=${hasSupplyInfo}, hasLiquidity=${hasLiquidity}, sizeUSD=${reserve.size?.usd}`);
                ...
                const pool = {
                    ...
                    token: {
                        ...
                        logoUrl: "logoUrl" in tokenMetadata
                    ...
                    wrappedTokenGatewayAddress: getAaveV3WrappedTokenGatewayAddress(market.chain.chainId) ||
                        undefined,
                };
