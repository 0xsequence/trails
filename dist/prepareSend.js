...
import { formatUsdValue, useAccountTokenBalance, formatValue, } from "./tokenBalances.js";
...
async function sendHandlerForDifferentChainDifferentToken({ mainSignerAddress, originChainId, originTokenAddress, originTokenAmount, destinationChainId, destinationTokenAddress, destinationTokenAmount, destinationTokenSymbol, recipient, destinationCalldata, apiClient, sourceTokenPriceUsd, destinationTokenPriceUsd, sourceTokenDecimals, destinationTokenDecimals, gasless, paymasterUrl, originRelayer, destinationRelayer, sequenceProjectAccessKey, relayerConfig, walletClient, publicClient, chain, account, fee, dryMode, onTransactionStateChange, transactionStates, slippageTolerance, tradeType, }) {
    ...
    const intentArgs = getIntentArgs(mainSignerAddress, originChainId, originTokenAddress, tradeType === TradeType.EXACT_INPUT
        ? destinationTokenAmount
        : originTokenAmount, // originTokenAmount
    destinationChainId, destinationTokenAddress, tradeType === TradeType.EXACT_INPUT ? "0" : destinationTokenAmount, // destinationTokenAmount
    ...
    const lastPrecondition = findFirstPreconditionForChainId(intent.preconditions, destinationChainId);
    const lastPreconditionMin = lastPrecondition?.data?.min?.toString();
    const originSendAmountFormatted = Number(formatUnits(BigInt(depositAmount), sourceTokenDecimals));
    const depositAmountUsd = originSendAmountFormatted * (sourceTokenPriceUsd ?? 0);
    const effectiveDestinationTokenAmount = tradeType === TradeType.EXACT_INPUT
        ? lastPreconditionMin
        : destinationTokenAmount;
    const effectiveDestinationTokenAmountFormatted = Number(formatUnits(BigInt(effectiveDestinationTokenAmount), destinationTokenDecimals));
    const effectiveDestinationTokenAmountUsd = effectiveDestinationTokenAmountFormatted * (destinationTokenPriceUsd ?? 0);
    return {
        intentAddress,
...
async function sendHandlerForSameChainSameToken({ originTokenAddress, destinationTokenAmount, destinationCalldata, recipient, walletClient, onTransactionStateChange, dryMode, account, chain, transactionStates, }) {
...
async function sendHandlerForSameChainDifferentToken({ originTokenAddress, swapAmount, destinationTokenAddress, destinationCalldata, recipient, originChainId, walletClient, publicClient, account, tradeType = TradeType.EXACT_OUTPUT, slippageTolerance, onTransactionStateChange, transactionStates, }) {
...
async function checkAccountBalance({ account, tokenAddress, depositAmount, publicClient, }) {
    try {
        ...
        if (!hasEnoughBalance) {
            balanceError = new InsufficientBalanceError(`Insufficient balance: Need ${formatValue(requiredAmountFormatted)} but only have ${formatValue(balanceFormatted)}`);
        }
        return {
            hasEnoughBalance,
            balance,
...
export function useQuote({ walletClient, fromTokenAddress, fromChainId, toTokenAddress, toChainId, swapAmount, tradeType, toRecipient, slippageTolerance, onStatusUpdate, } = {}) {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const { intentAddress, originSendAmount, destinationTokenAmount, send, fees, priceImpact, completionEstimateSeconds, transactionStates, } = await prepareSend(options);
            console.log("[trails-sdk] Intent address:", intentAddress?.toString());
            const quote = {
...
export function getSlippageToleranceFromIntent(_intent) {
    return "0";
}
export function getPriceImpactFromIntent(_intent) {
    return "0";
}
