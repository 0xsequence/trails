...
import { createPublicClient, createWalletClient, decodeFunctionData, erc20Abi, formatUnits, http, maxUint256, parseUnits, zeroAddress, } from "viem";
import { generatePrivateKey, privateKeyToAccount } from "viem/accounts";
import * as chains from "viem/chains";
import { trackPaymentCompleted, trackPaymentError, trackPaymentStarted, trackRelayerCallCompleted, trackRelayerCallError, trackRelayerCallStarted, trackTransactionConfirmed, } from "./analytics.js";
import { useAPIClient } from "./apiClient.js";
import { approveERC20, cctpTransfer, cctpTransferWithCustomCall, getCCTPRelayerCallData, getIsUsdcAddress, getMessageTransmitter, getMintUSDCData, getNeedsApproval, getUSDCTokenAddress, } from "./cctp.js";
...
import { executeSimpleRelayTransaction, getRelaySDKQuote, getTxHashFromRelayResult, waitForRelayDestinationTx, } from "./relaySdk.js";
import { getFeeOptions, sequenceSendTransaction, simpleCreateSequenceWallet, } from "./sequenceWallet.js";
import { formatUsdAmountDisplay, useAccountTokenBalance, formatAmount, formatRawAmount, formatAmountDisplay, } from "./tokenBalances.js";
import { getTokenInfo, useSupportedTokens, } from "./tokens.js";
...
import { getSequenceUseV3Relayers, getSequenceEnv, getSlippageTolerance, } from "./config.js";
...
function getTestnetOriginTokenAddress(testnetChainId) {
    return getUSDCTokenAddress(testnetChainId);
}
function getIntentArgs(mainSignerAddress, originChainId, originTokenAddress, originTokenAmount, destinationChainId, destinationTokenAddress, destinationTokenAmount, destinationTokenSymbol, recipient, destinationCalldata, destinationSalt = Date.now().toString(), slippageTolerance, // 0.03 = 3%
tradeType, provider) {
...
export async function prepareSend(options) {
    const { account, originTokenAddress, originChainId, originTokenBalance, // account balance
    destinationChainId, recipient, destinationTokenAddress, swapAmount, tradeType = TradeType.EXACT_OUTPUT, destinationTokenSymbol, fee, client: walletClient, dryMode = false, apiClient, originRelayer, destinationRelayer, destinationCalldata, onTransactionStateChange, sourceTokenPriceUsd, destinationTokenPriceUsd, sourceTokenDecimals, destinationTokenDecimals, paymasterUrl, gasless = false, slippageTolerance = getSlippageTolerance(), originNativeTokenPriceUsd, quoteProvider, fundMethod, } = options;
    const hasCustomCalldata = getIsCustomCalldata(destinationCalldata);
    ...
    if (hasCustomCalldata &&
        ...
        const wrapResult = wrapCalldataWithProxyCallerIfNeeded({
            ...
            calldata: effectiveDestinationCalldata,
            amount: swapAmount,
            originChainId,
            destinationChainId,
            originTokenAddress,
...
async function sendHandlerForDifferentChainDifferentToken({ mainSignerAddress, originChainId, originTokenAddress, originTokenBalance, destinationChainId, destinationTokenAddress, swapAmount, destinationTokenSymbol, recipient, destinationCalldata, apiClient, sourceTokenPriceUsd, destinationTokenPriceUsd, sourceTokenDecimals, destinationTokenDecimals, gasless, paymasterUrl, originRelayer, destinationRelayer, walletClient, publicClient, chain, account, fee, dryMode, onTransactionStateChange, transactionStates, slippageTolerance, tradeType, originNativeTokenPriceUsd, quoteProvider, fundMethod, }) {
    ...
    if (useCctp && cctpFlag) {
        ...
        return {
            ...
            send: async (onOriginSend) => {
                ...
                const calls = [];
                if (hasCustomCalldata) {
                    calls.push(await getCCTPRelayerCallData({
                        attestation,
                    ...
                    calls.push(await getMintUSDCData({
                        tokenMessenger,
                        attestation,
                    }));
                }
                ...
                const sequenceWalletAddress = await simpleCreateSequenceWallet(delegatorAccount);
                console.log("[trails-sdk] sequenceWalletAddress", sequenceWalletAddress);
                const sequenceTxHash = await sequenceSendTransaction(sequenceWalletAddress, delegatorClient, destinationPublicClient, calls, destinationChain);
    ...
    const intentArgs = getIntentArgs(mainSignerAddress, originChainId, originTokenAddress, tradeType === TradeType.EXACT_OUTPUT ? originTokenBalance : swapAmount, // originTokenAmount
    destinationChainId, destinationTokenAddress, tradeType === TradeType.EXACT_OUTPUT ? swapAmount : "0", // destinationTokenAmount
    destinationTokenSymbol, recipient, destinationCalldata, destinationSalt, slippageTolerance, tradeType, quoteProvider);
    ...
    const originSendAmountFormatted = formatRawAmount(depositAmount, sourceTokenDecimals);
    ...
    const effectiveDestinationTokenAmountFormatted = formatRawAmount(effectiveDestinationTokenAmount, destinationTokenDecimals);
    ...
    console.log("[trails-sdk] effectiveDestinationTokenAmountUsd", effectiveDestinationTokenAmountUsd, {
        amount: effectiveDestinationTokenAmountFormatted,
        usdPrice: destinationTokenPriceUsd,
    });
    return {
        ...
        send: async (onOriginSend) => {
            ...
            const checkForDepositTx = async () => {
                while (true) {
                    try {
                        ...
                        console.log("[trails-sdk] getAccountTransactionHistory response", response);
                        if (response.transactions.length > 0) {
                            ...
                            //   continue
                            // }
                            const originDepositTxReceipt = await publicClient.getTransactionReceipt({
                                hash: tx.txnHash,
                            });
                            originUserTxReceipt = originDepositTxReceipt;
                            transactionStates[0] = getTransactionStateFromReceipt(originDepositTxReceipt, originChainId, "Transfer");
                            onTransactionStateChange(transactionStates);
                            if (onOriginSend) {
                                onOriginSend();
                            }
            ...
            const destinationMetaTxnPromise = async () => {
                if (intent.quote.quoteProvider === "relay" &&
                    ...
                    try {
                        const txHash = await waitForRelayDestinationTx(intent.quote.quoteProviderRequestId);
                        ...
                        if (txHash) {
                            ...
                            const destinationTxnReceipt = await destinationPublicClient.getTransactionReceipt({
                                hash: txHash,
                            });
                            console.log("[trails-sdk] relay destinationTxnReceipt", destinationTxnReceipt);
                            transactionStates[2] = getTransactionStateFromReceipt(destinationTxnReceipt, destinationChainId, transactionStates[2]?.label);
                            onTransactionStateChange(transactionStates);
                        }
                    }
                    catch (error) {
                    ...
                    if (intent.metaTxns[1] && intent.preconditions[1]) {
                        ...
                        if (destinationMetaTxnReceipt && transactionStates[2]) {
                            transactionStates[2] = getTransactionStateFromReceipt(destinationMetaTxnReceipt, destinationChainId, transactionStates?.[2]?.label);
                            onTransactionStateChange(transactionStates);
                        }
                    }
                }
            };
            let queueCctpPromise = null;
            ...
            if (originUserTxReceipt && destinationMetaTxnReceipt) {
                trackPaymentCompleted({
                    userAddress: account.address,
                    intentAddress,
                    originTxHash: originUserTxReceipt
                        .transactionHash,
                    destinationTxHash: destinationMetaTxnReceipt
                        ?.txnHash,
                    originChainId,
                    destinationChainId,
                });
            ...
            return {
                ...
                totalCompletionSeconds: await getTxTimeDiff(transactionStates[0], transactionStates[2]),
            };
        },
    };
}
async function sendHandlerForSameChainSameToken({ originTokenAddress, swapAmount, destinationCalldata, recipient, walletClient, onTransactionStateChange, dryMode, account, chain, transactionStates, sourceTokenPriceUsd, destinationTokenPriceUsd, originNativeTokenPriceUsd, slippageTolerance, }) {
    ...
    return {
        ...
        send: async (onOriginSend) => {
            ...
            const originCallParams = {
                ...
                value: effectiveOriginTokenAddress === zeroAddress
                    ? BigInt(swapAmount)
                    : "0",
            ...
            if (!dryMode) {
                ...
                onTransactionStateChange([
                    getTransactionStateFromReceipt(originUserTxReceipt, effectiveOriginChainId, "Swap"),
                ]);
...
async function sendHandlerForSameChainDifferentToken({ originTokenAddress, swapAmount, destinationTokenAddress, destinationCalldata, recipient, originChainId, walletClient, publicClient, account, tradeType = TradeType.EXACT_OUTPUT, slippageTolerance, onTransactionStateChange, transactionStates, sourceTokenPriceUsd, destinationTokenPriceUsd, originNativeTokenPriceUsd, }) {
    ...
    const depositOriginAddress = quote?.steps?.[quote?.steps?.length - 1]?.items?.[quote?.steps?.[quote?.steps?.length - 1]?.items?.length - 1]?.data?.to;
    if (quote?.details?.currencyIn?.amountFormatted &&
        quote?.details?.currencyIn?.amountUsd) {
        const quoteProviderSourceTokenPriceUsd = Number(quote.details.currencyIn.amountUsd) /
    ...
    if (quote?.details?.currencyOut?.amountFormatted &&
        ...
        const quoteProviderDestinationTokenPriceUsd = Number(quote.details.currencyOut.amountUsd) /
    ...
    return {
        ...
        send: async (onOriginSend) => {
            ...
            transactionStates[0] = getTransactionStateFromReceipt(originUserTxReceipt, originChainId, "Swap");
            onTransactionStateChange(transactionStates);
...
async function attemptGaslessDeposit({ paymasterUrl, depositTokenAddress, depositTokenAmount, depositRecipient, onOriginSend, walletClient, chain, account, originRelayer, }) {
    let originUserTxReceipt = null;
    ...
    if (paymasterUrl) {
        ...
        const sequenceTxHash = await sequenceSendTransaction(sequenceWalletAddress, delegatorClient, publicClient, calls, chain);
...
async function attemptUserDepositTx({ originTokenAddress, gasless, paymasterUrl, chain, account, originRelayer, firstPreconditionMin, intentAddress, onOriginSend, publicClient, walletClient, destinationTokenDecimals, sourceTokenDecimals, fee, dryMode, sourceTokenPriceUsd, destinationTokenPriceUsd, swapAmount, onTransactionStateChange, transactionStates, fundMethod, }) {
    let originUserTxReceipt = null;
    const originChainId = chain.id;
    // Skip wallet deposit if fund method is qr-code
    if (fundMethod === "qr-code" || fundMethod === "exchange") {
...
function getTransactionStateFromReceipt(receipt, chainId, label = "Transaction") {
    let txHash = "";
    let state = "pending";
    let blockNumber = 0;
...
async function sendMetaTxAndWaitForReceipt({ metaTx, relayer, precondition, }) {
    try {
        ...
        trackRelayerCallStarted({
            ...
            contractAddress: metaTx.contract,
...
async function checkAccountBalance({ account, tokenAddress, depositAmount, publicClient, }) {
    try {
        ...
        if (!hasEnoughBalance) {
            balanceError = new InsufficientBalanceError(`Insufficient balance: Need ${formatAmount(requiredAmountFormatted)} but only have ${formatAmount(balanceFormatted)}`);
        }
        return {
            hasEnoughBalance,
            balance,
...
export function useQuote({ walletClient, fromTokenAddress, fromChainId, toTokenAddress, toChainId, swapAmount, tradeType, toRecipient, slippageTolerance, onStatusUpdate, } = {}) {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const swap = async () => {
                const { originUserTxReceipt, destinationMetaTxnReceipt, totalCompletionSeconds, } = await send();
                return {
                    originTransaction: {
                        transactionHash: originUserTxReceipt?.transactionHash,
                        explorerUrl: getExplorerUrl({
                            txHash: originUserTxReceipt?.transactionHash,
                            chainId: fromChainId,
                        }),
                    ...
                    destinationTransaction: {
                        ...
                        explorerUrl: getExplorerUrl({
                            txHash: destinationMetaTxnReceipt?.txnHash,
                            chainId: toChainId,
                        }),
...
export function getFeesFromIntent(intent, { tradeType, fromAmountUsd, toAmountUsd, }) {
    // TODO: remove this once API types are regenerated
    const totalFeeAmountUsd = intent?.quote?.quoteProviderFeeUsd ?? 0;
...
export function getSlippageToleranceFromIntent(intent) {
    return intent.quote?.maxSlippage?.toString() ?? "0";
}
export function getPriceImpactFromIntent(intent) {
    return intent?.quote?.priceImpact?.toString() ?? "0";
}
export function getPriceImpactUsdFromIntent(intent) {
    // Temporary type assertion until API types are regenerated
    return intent?.quote?.priceImpactUsd?.toString() ?? "0";
}
export function getFeesFromRelaySdkQuote(quote) {
...
export function getSlippageToleranceFromRelaySdkQuote(quote) {
    return (Number(quote?.details?.slippageTolerance?.origin?.percent ?? "0") / 100).toString();
}
export function getPriceImpactFromRelaySdkQuote(quote) {
    return (quote?.details?.swapImpact?.percent ?? 0).toString();
}
export function getPriceImpactUsdFromRelaySdkQuote(quote) {
    return (quote?.details?.swapImpact?.usd ?? 0).toString();
}
export function getZeroFees() {
    return {
...
export function getCompletionEstimateSeconds({ originChainId, destinationChainId, }) {
    if (originChainId === chains.mainnet.id &&
...
export function getIsCustomCalldata(calldata) {
...
export async function getNormalizedQuoteObject({ originAddress, destinationAddress, destinationCalldata, originChainId, destinationChainId, originAmount, originAmountMin, destinationAmount, destinationAmountMin, originTokenAddress, destinationTokenAddress, originTokenPriceUsd, destinationTokenPriceUsd, transactionStates, fees, slippageTolerance, priceImpact, priceImpactUsd, originNativeTokenPriceUsd, quoteProvider, }) {
    ...
    const destinationToken = await getTokenInfo(destinationChainId, destinationTokenAddress);
    ...
    const originAmountMinFormatted = formatRawAmount(originAmountMin || "0", originToken.decimals);
    const originAmountMinUsdFormatted = formatAmount(calcAmountUsdPrice({
        amount: originAmountMinFormatted,
        usdPrice: originTokenPriceUsd,
    }));
    const originAmountMinUsdDisplay = formatUsdAmountDisplay(originAmountMinUsdFormatted);
    const destinationAmountMinFormatted = formatRawAmount(destinationAmountMin || "0", destinationToken.decimals);
    const destinationAmountMinUsdFormatted = formatAmount(calcAmountUsdPrice({
        amount: destinationAmountMinFormatted,
        usdPrice: destinationTokenPriceUsd,
    }));
    const destinationAmountMinUsdDisplay = formatUsdAmountDisplay(destinationAmountMinUsdFormatted);
    const originAmountFormatted = formatRawAmount(originAmount, originToken.decimals);
    const originAmountUsdFormatted = formatAmount(calcAmountUsdPrice({
        amount: originAmountFormatted,
        usdPrice: originTokenPriceUsd,
    }));
    const originAmountUsdDisplay = formatUsdAmountDisplay(originAmountUsdFormatted);
    const destinationAmountFormatted = formatRawAmount(destinationAmount, destinationToken.decimals);
    const destinationAmountUsdFormatted = formatAmount(calcAmountUsdPrice({
        amount: destinationAmountFormatted,
        usdPrice: destinationTokenPriceUsd,
    }));
    const destinationAmountUsdDisplay = formatUsdAmountDisplay(destinationAmountUsdFormatted);
    const hasCustomCalldata = getIsCustomCalldata(destinationCalldata);
    const publicClient = createPublicClient({
        chain: originChain,
        transport: http(),
    ...
    let gasCostUsd = 0;
    let gasCostUsdDisplay = "0";
    try {
        if (originNativeTokenPriceUsd) {
            gasCostUsd = await estimateGasCostUsd(publicClient, originNativeTokenPriceUsd, 200000n);
    ...
    const destinationTokenPrice = destinationTokenPriceUsd
        ...
        : 0;
    const exchangeRates = getTokenExchangeRates(originTokenPrice, destinationTokenPrice, originToken.symbol, destinationToken.symbol);
    ...
    const quoteProviderName = quoteProviderNames[quoteProvider] ||
        quoteProvider ||
        "";
    const quoteProviderInfo = {
        id: quoteProvider || "",
    ...
    return {
        ...
        destinationAmountMinDisplay: formatAmountDisplay(destinationAmountMinFormatted),
        quoteProvider: quoteProviderInfo,
    };
}
function getTokenExchangeRates(originTokenPrice, destinationTokenPrice, originTokenSymbol, destinationTokenSymbol) {
