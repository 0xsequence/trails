...
// API Credentials
const DEFAULT_SANDBOX_API_KEY = "c2tfc2FuZF9rcmFrd3d1Zi5qY3d6ZG80bDM2ZGo0bHNtcHduazNkYm9hYmlxb2QyZWoxb2drM2dsaTE0NnphazB1cnhhcHh1MmFvcGVsZGxs";
const DEFAULT_PRODUCTION_API_KEY = "c2tfcHJvZF81MHh0M3BmNC5kYmx6OTdjY3VrdzVuMW92cDNmZ2F6aHNjZGh3a3NscW5zdGR2OGRmdGg4NWc2c2F0bWVvNDh5enF3NGt2bnZn";
const DEFAULT_CLIENT_ID = "018880d9-425c-49fa-b0e5-08ddce68a08d";
const DEFAULT_ENVIRONMENT = "production";
export function getMeshConnectApiKey(environment = "sandbox") {
    return atob(environment === "production"
        ? DEFAULT_PRODUCTION_API_KEY
        : DEFAULT_SANDBOX_API_KEY);
}
export function getMeshConnectApiUrl(environment = "sandbox") {
 ...
 */
function buildPayload(config, options = {}) {
    const payload = {
 ...
 */
async function makeApiRequest(payload, config) {
    ...
    try {
        ...
        const meshError = new Error(`Network error: ${error}`);
        throw meshError;
    }
}
/**
 ...
 */
export async function checkApiHealth(config) {
    const apiUrl = getMeshConnectApiUrl(config.environment);
 ...
 */
export async function createDefaultLinkToken(options = {
    environment: "sandbox",
}) {
 ...
 */
export async function createNewLinkToken(apiKey, clientId, options = {
    environment: "sandbox",
}) {
    const config = {
        apiKey,
 ...
 */
export async function createLinkToken(config, options = {}) {
    // Validate required parameters
 ...
 */
export async function getMeshNetworkIdFromChainId(chainId, environment = "sandbox") {
    try {
        ...
        const network = networks.find((net) => net.chainId === targetChainId);
        return network?.id;
    }
    catch (error) {
        console.error("[meshconnect] Failed to fetch network ID for chain ID:", chainId, error);
        return undefined;
    }
}
/**
 ...
 */
export function extractLinkToken(response) {
...
export function getMeshConnectClientId() {
    return DEFAULT_CLIENT_ID;
}
export function getMeshConnectEnvironment() {
...
export function getMeshConnectNetworkId(chainId) {
