...
const WidgetInner = forwardRef(({ appId: sequenceProjectAccessKey, sequenceIndexerUrl, sequenceApiUrl, sequenceEnv, toAddress, toAmount, toChainId, toToken, toCalldata, children, renderInline = true, theme: initialTheme = "auto", walletOptions, onOriginConfirmation, onDestinationConfirmation, useSourceTokenForButtonText, paymasterUrls, gasless, buttonText, }, ref) => {
    ...
    const [sendFormQuote, setSendFormQuote] = useState(null);
    ...
    const renderScreenContent = () => {
        switch (currentScreen) {
            ...
            case "wallet-confirmation":
                return (_jsx(WalletConfirmation, { onBack: handleBack, onComplete: () => setCurrentScreen("pending"), theme: theme, amount: sendFormQuote?.amount ?? undefined, amountUsd: sendFormQuote?.amountUsd ?? undefined, recipient: sendFormQuote?.intentAddress ?? "", tokenSymbol: selectedToken?.symbol, retryEnabled: showWalletConfirmRetry, onRetry: handleWalletConfirmRetry, fromTokenSymbol: sendFormQuote?.tokenSymbol || "", fromChainId: sendFormQuote?.chainId || 0, fromTokenImageUrl: sendFormQuote?.imageUrl || "", fees: sendFormQuote?.fees, slippageTolerance: sendFormQuote?.slippageTolerance, priceImpact: sendFormQuote?.priceImpact, destinationTokenSymbol: sendFormQuote?.destinationTokenSymbol, destinationTokenAmount: sendFormQuote?.destinationTokenAmount, destinationTokenAmountUsd: sendFormQuote?.destinationTokenAmountUsd, destinationChainId: sendFormQuote?.destinationChainId, destinationTokenImageUrl: sendFormQuote?.destinationTokenImageUrl }));
            case "pending":
                return (_jsx(TransferPending, { onComplete: handleTransferComplete, theme: theme, transactionStates: transactionStates, fromAmount: sendFormQuote?.amount || "", fromAmountUsd: sendFormQuote?.amountUsd || "", fromTokenSymbol: sendFormQuote?.tokenSymbol || "", fromTokenName: sendFormQuote?.tokenName || "", fromChainId: sendFormQuote?.chainId || 0, fromTokenImageUrl: sendFormQuote?.imageUrl || "" }));
            case "receipt":
