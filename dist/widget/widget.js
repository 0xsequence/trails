...
const WidgetInner = ({ sequenceProjectAccessKey, sequenceIndexerUrl, sequenceApiUrl, sequenceEnv, toAddress, toAmount, toChainId, toToken, toCalldata, children, renderInline = true, theme: initialTheme = "auto", walletOptions, onOriginConfirmation, onDestinationConfirmation, useSourceTokenForButtonText, paymasterUrls, gasless, }) => {
    ...
    const renderScreenContent = () => {
        switch (currentScreen) {
            ...
            case "tokens":
                return (_jsx(TokenList, { onContinue: handleTokenSelect, onBack: handleBack, indexerGatewayClient: indexerGatewayClient, theme: theme, targetAmountUsd: targetAmountUsd, targetAmountUsdFormatted: targetAmountUsdFormatted }));
            case "send":
            ...
            case "wallet-confirmation":
                return (_jsx(WalletConfirmation, { onBack: handleBack, onComplete: () => setCurrentScreen("pending"), theme: theme, amount: originTokenInfo?.amount ?? undefined, recipient: intentAddress ?? "", tokenSymbol: selectedToken?.symbol, retryEnabled: showWalletConfirmRetry, onRetry: handleWalletConfirmRetry }));
            case "pending":
...
export const TrailsWidget = (props) => {
