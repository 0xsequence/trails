...
const WidgetInner = forwardRef(({ appId: sequenceProjectAccessKey, sequenceIndexerUrl, sequenceApiUrl, sequenceEnv, toAddress, toAmount, toChainId, toToken, toCalldata, children, renderInline = true, theme: initialTheme = "auto", mode = "pay", walletOptions, onOriginConfirmation, onDestinationConfirmation, paymasterUrls, gasless, buttonText, }, ref) => {
    ...
    const renderScreenContent = () => {
        switch (currentScreen) {
            ...
            case "tokens":
                return (_jsx(TokenList, { onContinue: handleTokenSelect, onBack: handleBack, indexerGatewayClient: indexerGatewayClient, theme: theme, targetAmountUsd: targetAmountUsd, targetAmountUsdFormatted: targetAmountUsdFormatted, onError: handleTokenListError, mode: mode }));
            case "send-form":
                return selectedToken && walletClient?.account ? (_jsx(PaySendForm, { onSend: handleOnSend, onBack: handleBack, onWaitingForWalletConfirm: handleWaitingForWalletConfirm, onConfirm: () => setCurrentScreen("pending"), onComplete: handleTransferComplete, selectedToken: selectedToken, account: walletClient.account, sequenceProjectAccessKey: sequenceProjectAccessKey, apiUrl: sequenceApiUrl || undefined, env: sequenceEnv, toRecipient: toAddress || undefined, toAmount: toAmount || undefined, toChainId: toChainId ? Number(toChainId) : undefined, toToken: toToken || undefined, toCalldata: toCalldata || undefined, walletClient: walletClient, theme: theme, onTransactionStateChange: handleTransactionStateChange, onError: handleSendError, paymasterUrls: paymasterUrls, gasless: gasless, setWalletConfirmRetryHandler: setWalletConfirmRetryHandler })) : (_jsx("div", { className: `text-center p-4 rounded-lg ${theme === "dark"
                        ? "text-gray-300 bg-gray-800"
                        : "text-gray-600 bg-gray-50"}`, children: "Please connect wallet" }));
            case "fund-form":
                return selectedToken && walletClient?.account ? (_jsx(FundSendForm, { onSend: handleOnSend, onBack: handleBack, onWaitingForWalletConfirm: handleWaitingForWalletConfirm, onConfirm: () => setCurrentScreen("pending"), onComplete: handleTransferComplete, selectedToken: selectedToken, account: walletClient.account, sequenceProjectAccessKey: sequenceProjectAccessKey, apiUrl: sequenceApiUrl || undefined, env: sequenceEnv, toAmount: toAmount || undefined, toRecipient: toAddress || undefined, toChainId: toChainId ? Number(toChainId) : undefined, toToken: toToken || undefined, walletClient: walletClient, theme: theme, onTransactionStateChange: handleTransactionStateChange, onError: handleSendError, paymasterUrls: paymasterUrls, gasless: gasless, setWalletConfirmRetryHandler: setWalletConfirmRetryHandler, toCalldata: toCalldata || undefined })) : (_jsx("div", { className: `text-center p-4 rounded-lg ${theme === "dark"
                        ? "text-gray-300 bg-gray-800"
                        : "text-gray-600 bg-gray-50"}`, children: "Please connect wallet" }));
            case "wallet-confirmation":
                return (_jsx(WalletConfirmation, { onBack: handleBack, onComplete: handleWalletConfirmComplete, theme: theme, retryEnabled: showWalletConfirmRetry, onRetry: handleWalletConfirmRetry, quote: prepareSendQuote }));
            case "pending":
                return (_jsx(TransferPending, { onElapsedTime: handleElapsedTime, theme: theme, transactionStates: transactionStates, quote: prepareSendQuote }));
            case "receipt":
                return (_jsx(Receipt, { onSendAnother: handleSendAnother, onClose: handleCloseModal, theme: theme, renderInline: renderInline, transactionStates: transactionStates, totalCompletionSeconds: totalCompletionSeconds ?? undefined, quote: prepareSendQuote }));
            default:
                return null;
        }
    };
    const renderScreen = () => {
        return (_jsxs(motion.div, { initial: { opacity: 0, scale: 0.95 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: 0.95 }, transition: {
                ...
                damping: 30,
                mass: 1,
            }, className: `flex flex-col min-h-[400px] rounded-[32px] shadow-xl p-4 sm:p-6 relative w-full sm:w-[400px] mx-auto custom-scrollbar ${theme === "dark"
    ...
    return (_jsxs("div", { className: "flex flex-col items-center justify-center space-y-8 py-12", children: [!children ? (_jsx(motion.button, { whileHover: { scale: 1.02 }, whileTap: { scale: 0.98 }, onClick: () => setIsModalOpen(true), className: `${theme === "dark"
                    ...
                    : "bg-blue-500 hover:bg-blue-600"} text-white cursor-pointer font-semibold py-3 px-6 rounded-[24px] shadow-sm transition-colors`, children: buttonText || (mode === "fund" ? "Fund" : "Pay") })) : (_jsx(motion.div, { whileHover: { scale: 1.02 }, whileTap: { scale: 0.98 }, className: "flex flex-col items-center justify-center", onClick: () => setIsModalOpen(true), children: children })), _jsx(AnimatePresence, { children: isModalOpen && (_jsx(Modal, { isOpen: isModalOpen, onClose: handleCloseModal, theme: theme, children: renderScreen() })) })] }));
});
