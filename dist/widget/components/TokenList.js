...
export const TokenList = ({ onContinue, onBack, indexerGatewayClient, theme = "light", targetAmountUsd, targetAmountUsdFormatted, onError, mode, }) => {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between relative", children: [_jsxs("div", { className: "flex items-center", children: [!targetAmountUsd && mode !== "fund" && (_jsx("button", { type: "button", onClick: onBack, className: `p-2 rounded-full transition-colors cursor-pointer ${theme === "dark"
                                    ? "hover:bg-gray-800 text-gray-400"
                                    : "hover:bg-gray-100 text-gray-600"}`, children: _jsx(ChevronLeft, { className: "h-6 w-6" }) })), _jsx("h2", { className: `text-lg font-semibold ${targetAmountUsd || mode === "fund" ? "text-left" : "text-center"} ${theme === "dark" ? "text-white" : "text-gray-900"}`, children: mode === "fund"
                                    ? "Fund with any token in your wallet"
                                    : targetAmountUsd
                                        ? `Pay ${targetAmountUsdFormatted} with:`
                                        : "Select Token" })] }), totalBalanceUsd > 0 && mode !== "fund" && (_jsxs("p", { className: `text-xs mr-8 ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`, children: ["Total balance: ", totalBalanceUsdFormatted] }))] }), _jsxs("div", { className: "relative", children: [_jsx("div", { className: "absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none", children: _jsx(Search, { className: `h-5 w-5 ${theme === "dark" ? "text-gray-500" : "text-gray-400"}` }) }), _jsx("input", { type: "text", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), placeholder: "Search tokens", className: `block w-full pl-10 pr-3 py-2 border rounded-[24px] focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${theme === "dark"
                ...
                filteredTokens.length === 0 && (_jsx("div", { className: `text-center py-4 rounded-lg ${theme === "dark" ? "bg-gray-800" : "bg-gray-50"}`, children: _jsx("p", { className: theme === "dark" ? "text-gray-400" : "text-gray-500", children: searchQuery.trim()
                        ...
                        : "No tokens found with balance greater than 0." }) })), _jsx("div", { className: `divide-y ${theme === "dark" ? "divide-gray-700/50" : "divide-gray-200"} max-h-[35vh] overflow-y-auto rounded-[16px] custom-scrollbar ${theme === "dark" ? "bg-gray-800/50" : "bg-white"}`, children: filteredTokensFormatted.map((token) => {
