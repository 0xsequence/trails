...
export const TokenList = ({ onContinue, onBack, indexerGatewayClient, targetAmountUsd, targetAmountUsdFormatted, onError, mode, recentTokens = [], onRecentTokenSelect, }) => {
    ...
    const { searchQuery, setSearchQuery, handleTokenSelect, filteredTokens, isLoadingSortedTokens, isTokenSelected, selectedToken, showContinueButton, filteredTokensFormatted: baseFilteredTokensFormatted, totalBalanceUsd, totalBalanceUsdFormatted, showInsufficientBalance, balanceError, } = useTokenList({
        onContinue,
        targetAmountUsd,
    ...
    return (_jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between relative", children: [_jsxs("div", { className: "flex items-center", children: [!targetAmountUsd && mode !== "fund" && (_jsx("button", { type: "button", onClick: onBack, className: "p-2 rounded-full transition-colors cursor-pointer hover:trails-hover-bg text-gray-400", children: _jsx(ChevronLeft, { className: "h-6 w-6" }) })), _jsx("h2", { className: `text-lg font-semibold ${targetAmountUsd || mode === "fund" ? "text-left" : "text-center"} ${"text-gray-900 dark:text-white"}`, children: mode === "fund"
                                    ...
                                    setIsChainDropdownOpen(!isChainDropdownOpen);
                                            ...
                                            ? "trails-bg-selected"
                                            : ""}`, children: _jsx(ChainImage, { chainId: chain.chainId, size: 32 }) }, chain.chainId)))] }))] })] }), _jsx(AnimatePresence, { children: !isLoadingSortedTokens && filteredRecentTokens.length > 0 && (_jsx(motion.div, { initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: "auto" }, exit: { opacity: 0, height: 0 }, transition: { duration: 0.05, ease: "easeOut" }, children: _jsx(RecentTokens, { recentTokens: filteredRecentTokens, onTokenSelect: handleRecentTokenSelect }) })) }), isLoadingSortedTokens && (_jsxs("div", { className: "text-center py-4", children: [_jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 mx-auto border-black dark:border-white" }), _jsx("p", { className: "mt-2 text-gray-500 dark:text-gray-400", children: "Loading your token balances..." })] })), !isLoadingSortedTokens &&
                !balanceError &&
                filteredTokens.length === 0 && (_jsx("div", { className: "text-center py-4 rounded-lg trails-bg-secondary", children: _jsx("p", { className: "text-gray-500 dark:text-gray-400", children: searchQuery.trim()
                        ...
                        return (_jsxs(motion.button, { layout: true, initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 }, transition: {
                                ...
                                : undefined, className: `w-full py-2.5 px-3 flex items-center space-x-3 transition-colors trails-list-item border-b trails-border-list ${isTokenSelected(token) ? "bg-gray-50 dark:bg-gray-800" : ""} ${!isSufficientBalance ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`, children: [_jsx("div", { className: "relative flex-shrink-0", children: _jsx("div", { className: `rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700`, children: contractAddress ? (_jsx(TokenImage, { symbol: symbol[0], imageUrl: imageUrl, chainId: chainId, size: 32 })) : (_jsx("span", { className: "text-base font-medium text-gray-600 dark:text-gray-300", children: symbol })) }) }), _jsxs("div", { className: "flex-1 min-w-0 text-left", children: [_jsx("h3", { className: `text-sm font-medium truncate ${"text-gray-900 dark:text-white"} ${!isSufficientBalance ? "text-gray-500 dark:text-gray-500" : ""}`, children: tokenName }), _jsx("p", { className: `text-xs ${"text-gray-500 dark:text-gray-400"} ${!isSufficientBalance ? "text-gray-600 dark:text-gray-600" : ""}`, children: symbol })] }), _jsx("div", { className: "text-right flex-shrink-0", children: priceUsd > 0 ? (_jsxs(_Fragment, { children: [_jsx("p", { className: `text-sm font-medium ${"text-gray-900 dark:text-white"} ${!isSufficientBalance ? "text-gray-500 dark:text-gray-500" : ""}`, children: balanceUsdFormatted }), _jsx("p", { className: `text-xs ${"text-gray-500 dark:text-gray-400"} ${!isSufficientBalance ? "text-gray-600 dark:text-gray-600" : ""}`, children: balanceFormatted })] })) : (_jsx("p", { className: `text-sm font-medium ${"text-gray-900 dark:text-white"} ${!isSufficientBalance ? "text-gray-500 dark:text-gray-500" : ""}`, children: balanceFormatted })) })] }, `${chainId}-${contractAddress}`));
                    }) }) })), showInsufficientBalance && (_jsxs("div", { className: `text-left py-3 px-4 rounded-lg ${"bg-amber-500/10 border border-amber-500/30"}`, children: [_jsx("p", { className: `text-xs font-medium ${"text-amber-400"}`, children: "Insufficient balance" }), _jsx("p", { className: `text-xs mt-1 ${"text-amber-300"}`, children: "You do not have enough funds to reach the target amount" })] })), showContinueButton && (_jsx("div", { className: "space-y-4", children: _jsx("button", { type: "button", onClick: () => selectedToken && onContinue(selectedToken), disabled: !selectedToken, className: `w-full font-semibold py-3 px-4 trails-border-radius-button transition-colors bg-blue-500 hover:bg-blue-600 disabled:bg-gray-300 text-white disabled:text-gray-500 disabled:cursor-not-allowed cursor-pointer`, children: "Continue" }) }))] }));
};
