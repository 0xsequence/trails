...
import { createNewLinkToken, getMeshConnectClientId, getMeshConnectEnvironment, getMeshConnectApiKey, getMeshNetworkIdFromChainId, } from "../../meshconnect.js";
export const MeshConnect = ({ onBack, toTokenSymbol, toTokenAmount, toChainId, toRecipientAddress, onComplete, }) => {
    ...
    const [iframePlaceholder, setIframePlaceholder] = useState(null);
    ...
    useEffect(() => {
        const generateLinkToken = async () => {
            try {
                ...
                const networkId = (await getMeshNetworkIdFromChainId(toChainId, getMeshConnectEnvironment()));
                // Generate a new link token for receiving tokens
                const response = await createNewLinkToken(getMeshConnectApiKey(getMeshConnectEnvironment()), getMeshConnectClientId(), {
                ...
                console.log("[trails-sdk] Generated Mesh Connect link token response:", response);
                ...
                console.error("[trails-sdk] Failed to generate Mesh Connect link token:", err);
    ...
    const handleIntegrationConnected = useCallback((authData) => {
    ...
    const handleTransferFinished = useCallback((transferData) => {
    ...
    useEffect(() => {
        const createMeshLink = async () => {
            try {
                ...
                setError(`Failed to load Mesh Connect SDK: ${err instanceof Error ? err.message : String(err)}`);
            }
        };
        createMeshLink();
    ...
    const openMeshConnect = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            try {
                ...
                window.meshLink = meshLink;
            }
            catch (iframeError) {
                console.warn("[trails-sdk] Failed to open in iframe, trying popup:", iframeError);
                try {
                    // Fallback to popup
    ...
    useEffect(() => {
        if (transferFinishedData) {
            console.log("[trails-sdk] Transfer finished data updated:", transferFinishedData);
    ...
    if (loading) {
        return (_jsxs("div", { className: "flex flex-col h-full", children: [_jsxs("div", { className: "flex items-center justify-between mb-6", children: [_jsx("button", { type: "button", onClick: onBack, className: "p-2 -ml-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer", children: _jsx(ArrowLeft, { className: "w-5 h-5" }) }), _jsx("h2", { className: "text-lg font-semibold", children: "Mesh Connect" }), _jsx("div", { className: "w-9" })] }), _jsx("div", { className: "flex-1 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" }), _jsx("p", { className: "text-gray-600 dark:text-gray-400", children: "Generating secure connection..." })] }) })] }));
    }
    if (error) {
        return (_jsxs("div", { className: "flex flex-col h-full", children: [_jsxs("div", { className: "flex items-center justify-between mb-6", children: [_jsx("button", { type: "button", onClick: onBack, className: "p-2 -ml-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors", children: _jsx(ArrowLeft, { className: "w-5 h-5" }) }), _jsx("h2", { className: "text-lg font-semibold", children: "Mesh Connect" }), _jsx("div", { className: "w-9" })] }), _jsx("div", { className: "flex-1 flex items-center justify-center", children: _jsxs("div", { className: "text-center p-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20 dark:border-red-800", children: [_jsx("p", { className: "text-red-600 dark:text-red-200 mb-4", children: error }), _jsx("button", { type: "button", onClick: () => {
                                    if (typeof window !== "undefined") {
                                        window.location.reload();
                                    }
                                }, className: "px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors", children: "Try Again" })] }) })] }));
    }
    return (_jsxs("div", { className: "flex flex-col h-full", children: [_jsxs("div", { className: "flex items-center justify-between mb-6", children: [_jsx("button", { type: "button", onClick: onBack, className: "p-2 -ml-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors", children: _jsx(ArrowLeft, { className: "w-5 h-5" }) }), _jsx("h2", { className: "text-lg font-semibold", children: "Fund with Mesh Connect" }), _jsx("div", { className: "w-9" })] }), transferFinishedData && (_jsxs("div", { className: "mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg", children: [_jsxs("p", { className: "text-sm text-blue-600 dark:text-blue-200", children: ["\u2713 Transfer completed: ", transferFinishedData.amount, " ", transferFinishedData.symbol] }), _jsx("p", { className: "text-xs text-blue-500 dark:text-blue-300 mt-1", children: "Redirecting to pending screen..." })] })), _jsx("div", { className: "flex-1", children: !showIframe && !payload ? (_jsx("div", { className: "flex items-center justify-center h-full min-h-[500px]", children: _jsxs("div", { className: "text-center p-8 border border-gray-200 dark:border-gray-700 rounded-lg", children: [_jsx("h3", { className: "text-lg font-semibold mb-4", children: "Fund your account with Mesh Connect" }), _jsx("p", { className: "text-gray-600 dark:text-gray-400 mb-6", children: "Connect to bank account or exchange to fund your wallet securely." }), _jsx("button", { onClick: openMeshConnect, disabled: !meshLink || !linkToken, className: "px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed", children: !meshLink || !linkToken ? "Preparing..." : "Continue" })] }) })) : showIframe ? (_jsxs("div", { className: "relative", children: [_jsx("button", { onClick: handleCloseMeshConnect, className: "absolute top-2 right-2 z-10 bg-gray-800 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-gray-700", children: "\u00D7" }), _jsx("div", { ref: setIframePlaceholder, className: "w-full h-[500px] border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800" })] })) : null }), getMeshConnectEnvironment() === "sandbox" && (_jsx("div", { className: "p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800", children: _jsxs("div", { className: "text-center", children: [_jsx("p", { className: "text-sm font-medium text-yellow-800 dark:text-yellow-200", children: "Sandbox environment" }), _jsx("p", { className: "text-xs text-yellow-600 dark:text-yellow-300", children: "No real funds are used" })] }) })), iframeContainer &&
                showIframe &&
                createPortal(_jsx("div", { style: { width: "100%", height: "100%", position: "relative" }, children: _jsx("iframe", { id: "mesh-iframe", title: "Mesh Connect", style: {
                            width: "100%",
                            height: "100%",
                            border: "none",
                            borderRadius: "8px",
