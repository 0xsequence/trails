...
import { formatRawAmount, formatUsdAmountDisplay, formatAmount, formatAmountDisplay, } from "../../tokenBalances.js";
...
export function useSendForm({ account, toAmount, // Custom specified amount
...
walletClient, onTransactionStateChange, onError, onWaitingForWalletConfirm, paymasterUrls, gasless, selectedToken, onSend, onConfirm, onComplete, setWalletConfirmRetryHandler, tradeType = TradeType.EXACT_OUTPUT, quoteProvider, fundMethod, mode, onNavigateToMeshConnect, }) {
    const [amount, setAmount] = useState(tradeType === TradeType.EXACT_INPUT ? "" : (toAmount ?? ""));
    ...
    const { tokenPrices: destTokenPrices } = useTokenPrices(selectedDestToken && destTokenAddress
        ...
        : [], apiClient);
    const { tokenPrices: sourceTokenPrices } = useTokenPrices(selectedToken
        ? [
            {
    ...
    const [prepareSendResult, setPrepareSendResult] = useState(null);
    // Create a stable callback for transaction state changes
    const handleTransactionStateChange = useCallback((transactionStates) => {
        // Pass transaction states to widget-level handler
        onTransactionStateChange(transactionStates);
    }, [onTransactionStateChange]);
    const balanceFormatted = formatRawAmount(selectedToken.balance, selectedToken.contractInfo?.decimals);
    ...
    const amountUsdDisplay = useMemo(() => {
        const tokenPrice = tradeType === TradeType.EXACT_INPUT
    ...
    const amountRaw = useMemo(() => {
        ...
        const decimals = tradeType === TradeType.EXACT_INPUT
    ...
    const getQuote = useCallback(async () => {
        ...
        try {
            ...
            const destinationRelayer = getRelayer(undefined, selectedDestinationChain.id);
            ...
            if (!destinationTokenPriceUsd) {
                try {
                    ...
                    console.error("[trails-sdk] Error getting destination token price:", error);
                }
            }
            if (!destinationTokenPriceUsd &&
            ...
            if (selectedToken.contractAddress === zeroAddress &&
                sourceTokenPriceUsd) {
            ...
            const options = {
                ...
                originTokenBalance: fundMethod === "qr-code" || fundMethod === "exchange"
                    ? "1"
    ...
    const processSend = useCallback(async () => {
        try {
            ...
            if (fundMethod === "exchange" && onNavigateToMeshConnect) {
                ...
                const toTokenAmount = normalizeNumber(quote.originAmountFormatted).toString(); // MeshConnect will deposit origin token amount
                ...
                if (originChainId === destinationChainId) {
                    throw new Error("[trails-sdk] Must be different chain than the origin chain to use mesh-connect");
                }
                if (!toTokenSymbol ||
                    !toTokenAmount ||
                    !toChainId ||
                    !toRecipientAddress) {
                    throw new Error("[trails-sdk] Missing required props for mesh-connect");
                }
    ...
    const buttonText = useMemo(() => {
        ...
        const amountFormatted = prepareSendResult?.quote?.originAmountFormatted ?? formatAmount(amount);
        const destinationAmountFormatted = prepareSendResult?.quote?.destinationAmountFormatted ??
            formatAmount(amount);
        const tokenSymbol = selectedToken.symbol;
        const destinationTokenSymbol = selectedDestToken?.symbol;
        const amountDisplay = formatAmountDisplay(amountFormatted);
        const destinationAmountDisplay = formatAmountDisplay(destinationAmountFormatted);
        try {
