...
function getDestTokenAddress(chainId, tokenSymbol) {
    if (tokenSymbol === "ETH") {
        return zeroAddress;
    }
    if (chainId === 1) {
...
export function useSendForm({ account, sequenceProjectAccessKey, apiUrl, env, toAmount, toRecipient, toChainId, toToken, toCalldata, walletClient, onTransactionStateChange, useSourceTokenForButtonText, onError, onWaitingForWalletConfirm, paymasterUrls, gasless, selectedToken, onSend, onConfirm, onComplete, setWalletConfirmRetryHandler, }) {
    ...
    const [selectedChain, setSelectedChain] = useState(() => SUPPORTED_TO_CHAINS.find((chain) => chain.id === (toChainId ?? selectedToken.chainId)) || SUPPORTED_TO_CHAINS[0]);
    const [isChainDropdownOpen, setIsChainDropdownOpen] = useState(false);
    const [isTokenDropdownOpen, setIsTokenDropdownOpen] = useState(false);
    const [selectedDestToken, setSelectedDestToken] = useState(() => toToken
    ...
    const { data: destTokenPrices } = useTokenPrices(selectedDestToken
        ? (() => {
            try {
                const contractAddress = getDestTokenAddress(selectedChain.id, selectedDestToken.symbol);
                return [
                    {
    ...
    useEffect(() => {
        if (toChainId) {
            const newChain = SUPPORTED_TO_CHAINS.find((chain) => chain.id === toChainId);
            if (newChain) {
    ...
    useEffect(() => {
        if (toToken) {
            const newToken = SUPPORTED_TO_TOKENS.find((token) => token.symbol === toToken);
            if (newToken) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                ...
                setError(`${selectedDestToken.symbol} is not available on ${selectedChain.name}`);
            ...
            onWaitingForWalletConfirm(intentAddress?.toString() ?? "", Number(formatUnits(BigInt(originSendAmount), selectedToken.contractInfo?.decimals ?? 18))
                .toFixed(5)
                .toString());
            ...
            if (toCalldata) {
                if (useSourceTokenForButtonText) {
                    ...
                    if (destPrice > 0 && sourcePrice > 0) {
                        ...
                        const formattedSourceAmount = sourceAmount.toLocaleString(undefined, {
