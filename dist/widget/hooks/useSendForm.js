...
walletClient, onTransactionStateChange, useSourceTokenForButtonText, onError, onWaitingForWalletConfirm, paymasterUrls, gasless, selectedToken, onSend, onConfirm, onComplete, setWalletConfirmRetryHandler, }) {
    ...
    const processSend = useCallback(async () => {
        try {
            ...
            const { intentAddress, originSendAmount, fees, slippageTolerance, priceImpact, send, } = await prepareSend(options);
            ...
            const destinationTokenAmountFormatted = Number(formatUnits(BigInt(parsedAmount), selectedDestToken.decimals ?? 18));
            const destinationTokenAmountUsdFormatted = destinationTokenAmountFormatted * (destinationTokenPriceUsd ?? 0);
            ...
            onWaitingForWalletConfirm({
                ...
                destinationTokenAmountUsd: formatUsdValue(destinationTokenAmountUsdFormatted),
                destinationChainId: selectedDestinationChain.id,
