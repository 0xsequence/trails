...
import { getSequenceProjectAccessKey, getSequenceEnv, getSequenceUseV3Relayers, } from "./config.js";
...
export async function simpleCreateSequenceWallet(account, relayerConfig = {
    env: getSequenceEnv(),
...
export async function getIsWalletDeployed(wallet, publicClient) {
...
export async function waitForWalletDeployment(wallet, publicClient) {
    while (true) {
...
export async function sequenceSendTransaction(sequenceWalletAddress, accountClient, publicClient, calls, chain, relayerConfig = {
    env: getSequenceEnv(),
    useV3Relayers: getSequenceUseV3Relayers(),
}, sequenceProjectAccessKey = getSequenceProjectAccessKey()) {
    ...
    const isDeployed = await getIsWalletDeployed(wallet.address, publicClient);
    if (!isDeployed) {
        ...
        if (feeOptions?.options.length === 0) {
            ...
            await waitForWalletDeployment(wallet.address, publicClient);
            ...
            if (option) {
                ...
                console.log("[trails-sdk] All transactions (deployment + fees):", allTransactions);
                ...
                console.log("[trails-sdk] signed transactions with fees:", signed.transactions);
                console.log("[trails-sdk] signed entrypoint with fees:", signed.entrypoint);
                const bytes = new Uint8Array(32);
                crypto.getRandomValues(bytes);
                wallet.relayer.relay({
                    entrypoint: deployTx.entrypoint,
