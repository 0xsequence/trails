  ...
  const sortedTokens = useMemo(() => {
    ...
    for (const balance of tokenBalancesData.balances) {
      const price = tokenPrices?.find(
        (p) => p.token.contractAddress === balance.contractAddress,
      );
      if (price) {
    ...
    for (const balance of tokenBalancesData.nativeBalances) {
      const price = tokenPrices?.find(
        (p) =>
          p.token.contractAddress === zeroAddress &&
          p.token.chainId === balance.chainId,
      );
      if (price) {
        balance.price = price.price;
      }
    ...
    return [...balances]
      ...
      .map((token) => {
        if (token.price) {
          ...
          token.balanceUsdFormatted = getTokenBalanceUsdFormatted(
            token,
            token.price,
          );
        }
