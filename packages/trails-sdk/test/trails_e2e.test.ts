...
async function prepareSend(options: SendOptions) {
  ...
  const apiClient = getAPIClient(
    "http://localhost:4422",
    process.env.SEQUENCE_API_KEY as string,
  )
    ...
    send: async () => {
      ...
      if (shouldSend) {
        ...
        const originCallParams = {
          ...
          data: "0x",
          value:
            BigInt(intent.preconditions[0].data.min) + BigInt("5600000000000"),
      ...
      while (true) {
        ...
        const receipt = await getMetaTxnReceipt(
          originRelayer,
          metaTx.id,
